...
#
# Runs nova.sh up and notifies ntfy only when containers are actually
# recreated/started, or when the run fails. Silent on healthy no-op runs.
...
set +o pipefail
NOVA_SUPPRESS_NOTIFY=1 "${NOVA_SH}" up 2>&1 | tee "${TMPLOG}"
RC=${PIPESTATUS[0]}
set -o pipefail
