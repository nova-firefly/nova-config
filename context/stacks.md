...
| gaming | docker-compose.gaming.yaml | minecraft, pterodactyl-db, pterodactyl-cache, pterodactyl-panel, pterodactyl-wings |
...
| Service | Image | Port(s) | URL | Notes |
|---------|-------|---------|-----|-------|
| minecraft | itzg/minecraft-server:java25 | 25565 | — | Vanilla Minecraft server; direct TCP |
...
**External volumes:** `minecraft_data`, `pterodactyl_db`, `pterodactyl_panel_var`, `pterodactyl_panel_logs`
...
**Note:** Wings runs with `privileged: true` and the host Docker socket mounted. It creates game server containers directly on the host Docker daemon. The existing `minecraft` service runs independently of Pterodactyl.
