...
- **Both loops tolerate a per-stack failure, and any new stack loop must too.** `nova.sh` runs under `set -euo pipefail`, so an unguarded `run_compose` inside the loop aborts the whole run at the first bad stack and silently skips every stack after it in `ALL_STACKS` — this is exactly what happened to `heal`: an unpullable image in `dev` (8th of 13) left `tools`, `movienight`, `movienight-test`, `strava-hevy` and `todoassist` unhealed for weeks, with nothing in the ntfy alert to say so. Wrap the call in `if ! ...; then`, collect the failed stack names, and exit non-zero at the end with `_NTFY_DETAIL` set so the push names them.
